...
// 2001-12-16 MAX_FPS cvar by MrG  start
	...
	if (max_fps.value < 0.1)	Cvar_SetValue ("max_fps", 100);
	if (!cls.timedemo && realtime - oldrealtime < 0.1/max_fps.value)
...
// check for new clients
	...
	while(temp_host_frametime > (0.1/72.0))
	{
		if (temp_host_frametime > 0.1)
			host_frametime = 0.1;
		else
...
// keep Con_Printf from trying to update the screen
	...
	if (cls.state != ca_dedicated)
	{
     ...
     void *subelf; 
      int length;	
    //   iso_ioctl(0,NULL,0);
         length = fs_load("/cd/radquake.bin", &subelf); // Reload mod menu from Quake menu with quit  Ian micheal 2019
