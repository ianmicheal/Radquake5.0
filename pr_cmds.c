...
void PF_ftos (void)
{
	...
	if (v == (int)v)
		...
		// BlackAura (08-12-2002) - Extra spaces in ftos() bugfix
		sprintf (pr_string_temp, "%1f", (double)v);
...
void PF_vtos (void)
{
	sprintf (pr_string_temp, "'%5.1f %5.1f %5.1f'", (double)G_VECTOR(OFS_PARM0)[0], (double)G_VECTOR(OFS_PARM0)[1], (double)G_VECTOR(OFS_PARM0)[2]);
	G_INT(OFS_RETURN) = pr_string_temp - pr_strings;
}
...
/* speud - save to config file  begin */
cvar_t	sv_aim_h = {"sv_aim_h", "1", true};
cvar_t	sv_aim = {"sv_aim", "0.93", true};
/* speud - save to config file  end */
