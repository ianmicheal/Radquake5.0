...
keyname_t keynames[] =
{
	...
	// BlackAura (09-12-2002) - DC controller
	{"DC_A", K_DC_A},
	{"DC_B", K_DC_B},
	{"DC_C", K_DC_C},
	{"DC_D", K_DC_D},
	...
	// BlackAura (11-12-2002) - Removed first 18 K_AUX buttons
	/*
	{"AUX1", K_AUX1},
	{"AUX2", K_AUX2},
	{"AUX3", K_AUX3},
...
void Key_Console (int key)
{
	...
	if (key == K_ENTER || key == K_DC_Y) // Manoel Kasimier - edited
	{
	...
	if (key == K_UPARROW || key == K_DPAD1_LEFT) // Manoel Kasimier - edited
	{
		do
		{
	...
	if (key == K_DOWNARROW || key == K_DPAD1_RIGHT) // Manoel Kasimier - edited
	{
	...
	if (key == K_PGUP || key==K_MWHEELUP || key == K_DPAD1_UP) // Manoel Kasimier - edited
	{
		con_backscroll += 2;
	...
	if (key == K_PGDN || key==K_MWHEELDOWN || key == K_DPAD1_DOWN) // Manoel Kasimier - edited
	{
		con_backscroll -= 2;
		if (con_backscroll < 0)
			con_backscroll = 0;
	...
	if (key == K_HOME || key == K_DC_RTRIG) // Manoel Kasimier - edited
	{
		con_backscroll = con_totallines - (vid.height>>3) - 1;
		return;
	}

	if (key == K_END || key == K_DC_LTRIG) // Manoel Kasimier - edited
	{
		con_backscroll = 0;
		return;
	}
...
void Key_Message (int key)
{
	...
	if (key == K_ESCAPE || key == K_DC_B || key == K_DC_START) // Manoel Kasimier - edited
	{
...
{
	int i;
	char tmp_buf[40000]; // added by speud

	for (i=0 ; i<256 ; i++)
		if (keybindings[i]) {
			if (*keybindings[i]) {
/* speud - replacing fprintf begin */
//				fprintf (f, "bind \"%s\" \"%s\"\n", Key_KeynumToString(i), keybindings[i]);
				sprintf (tmp_buf, "bind \"%s\" \"%s\"\n", Key_KeynumToString(i), keybindings[i]);
				fwrite (tmp_buf, strlen(tmp_buf), 1, f);
/* speud - replacing fprintf end */
			}
	}
}
